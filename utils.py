...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    a,b = zip(*datazip)
    n = (len(a)-1)//batch_size+1
    batched_data = np.array_split(a,n)
    batched_label = np.array_split(b,n)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    plt.title("learning Curves of Accuracy")
    plt.xlabel("Epochs")
    plt.ylabel("Accuracy")
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig('learning Curves of Accuracy.png')
    plt.close()

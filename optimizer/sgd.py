...
class SGD(_BaseOptimizer):
    ...
    def update(self, model):
        ...
        #     model.weights["W1"] -= self.learning_rate *\
        #                  model.gradients["W1"]

        # if "W2" in model.weights:
        #     for i in range(len(model.weights["W2"])):
        #         for j in range(len(model.weights["W2"][i])):
        #             model.weights["W2"][i][j] -= self.learning_rate *\
        #                  model.gradients["W2"][i][j]

        for key in model.gradients:
            model.weights[key] -= self.learning_rate * model.gradients[key]
        
        # for key in model.gradients:
        #     model.weights[key] -= self.learning_rate * model.weights[key]
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
